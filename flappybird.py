...
# Создаем окно и указываем его размеры
GAME_WIDTH = 360
GAME_HEIGHT = 640
...
pipes = []
velosity_x = -2 # скорость движения труб влево
velocity_y = 0
gravity = 0.4
score = 0
record = load_record()
...
def draw():
    ...
    record_str = "Рекорд: " + str(int(record))
    ...
    record_render = text_font.render(record_str, True, "white")
    window.blit(record_render, (5, 50))

def move():
    global velocity_y, score, game_over, record
    velocity_y += gravity
    bird.y += velocity_y
    bird.y = max(bird.y, 0)
